...
module.exports = function (grunt) {
  ...
  var extension = '.json';
  if(userManagementInterfaceConfigFile.substring(userManagementInterfaceConfigFile.length-extension.length) !== extension) {
	  userManagementInterfaceConfigFile += extension;
  }
  var userManagementInterfaceConfig = require(userManagementInterfaceConfigFile);
  userManagementInterfaceConfig.swVersion = bowerConfig.version;
  userManagementInterfaceConfig.projectName = bowerConfig.description;
  // Normalizing the value, in case it does not exist
  if(userManagementInterfaceConfig.useLocalExportServer===undefined) {
	userManagementInterfaceConfig.useLocalExportServer = false;
  }

  var useminPrepare;
  var buildTasks;
  var buildTasksFast;
  if(userManagementInterfaceConfig.debug) {
	useminPrepare = {
  ...
  grunt.initConfig({
    ...
    */
    ...
    template: {
      config: {
        options: {
          data: userManagementInterfaceConfig
        },
        files: {
